server_causas <- function(input, output, session) {
  ...
  # --- CLUSTERS DE PROVINCIAS (k-means sobre perfiles de causas) ---
  cluster_colores <- c("#66C2A5", "#FC8D62", "#8DA0CB", "#E78AC3",
                       "#A6D854", "#FFD92F", "#E5C494", "#B3B3B3")

  # FIX: la población se suma por (Provincia, Capítulo) sobre las filas de sexo
  # (idéntica en cada causa), igual que datos_p2; nunca sobre las causas.
  datos_cl_mat <- reactive({
    req(input$cl_ano, input$cl_sexo)
    df <- causas_provinciales %>% filter(Año == input$cl_ano)
    if (input$cl_sexo != "Ambos") df <- df %>% filter(Sexo == input$cl_sexo)
    wide <- df %>%
      group_by(Provincia, Comunidad, Defunción) %>%
      summarise(Fall = sum(Fallecidos, na.rm = TRUE),
                Pob = sum(Poblacion, na.rm = TRUE), .groups = "drop") %>%
      mutate(Tasa = if_else(Pob > 0, Fall / Pob * 100000, NA_real_)) %>%
      filter(is.finite(Tasa)) %>%
      select(Provincia, Comunidad, Defunción, Tasa) %>%
      tidyr::pivot_wider(names_from = Defunción, values_from = Tasa, values_fill = 0)
    mat <- as.matrix(wide[, setdiff(names(wide), c("Provincia", "Comunidad"))])
    rownames(mat) <- wide$Provincia
    vars <- apply(mat, 2, stats::sd, na.rm = TRUE)
    mat <- mat[, vars > 0, drop = FALSE]
    list(geo = wide[, c("Provincia", "Comunidad")], mat = mat,
         z = scale(mat), causas = colnames(mat))
  }) %>% bindCache(input$cl_ano, input$cl_sexo)

  silueta_media <- function(z, grupos) {
    if (!requireNamespace("cluster", quietly = TRUE)) return(NA_real_)
    suppressWarnings(tryCatch(
      mean(cluster::silhouette(grupos, stats::dist(z))[, "sil_width"]),
      error = function(e) NA_real_
    ))
  }

  datos_cl_km <- reactive({
    req(input$cl_k)
    d <- datos_cl_mat()
    req(nrow(d$z) >= input$cl_k, ncol(d$z) >= 1)
    set.seed(20260214)
    km <- stats::kmeans(d$z, centers = input$cl_k, nstart = 25)
    list(km = km, sil = silueta_media(d$z, km$cluster))
  }) %>% bindCache(input$cl_ano, input$cl_sexo, input$cl_k)

  datos_cl_codo <- reactive({
    d <- datos_cl_mat()
    req(nrow(d$z) >= 8)
    ks <- 2:8
    set.seed(20260214)
    wss <- vapply(ks, function(k) stats::kmeans(d$z, centers = k, nstart = 10)$tot.withinss,
                  numeric(1))
    set.seed(20260214)
    sil <- vapply(ks, function(k) {
      km <- stats::kmeans(d$z, centers = k, nstart = 10)
      silueta_media(d$z, km$cluster)
    }, numeric(1))
    data.frame(k = ks, WSS = wss, Silueta = sil)
  }) %>% bindCache(input$cl_ano, input$cl_sexo)

  mejor_k_silueta <- function(codo) {
    if (!nrow(codo) || all(is.na(codo$Silueta))) return(NA_integer_)
    codo$k[which.max(codo$Silueta)]
  }

  output$cl_kpi_k <- renderText({
    d <- datos_cl_mat()
    paste0(input$cl_k, " clusters · ", nrow(d$z), " provincias")
  })

  output$cl_kpi_sil <- renderText({
    codo <- datos_cl_codo()
    k <- mejor_k_silueta(codo)
    if (is.na(k)) return("-")
    paste0("k = ", k, " (",
           format(round(codo$Silueta[codo$k == k], 2), decimal.mark = ","), ")")
  })

  output$cl_kpi_top <- renderText({
    km <- datos_cl_km()$km
    tab <- sort(table(km$cluster), decreasing = TRUE)
    paste0("Cluster ", names(tab)[1], " (", as.integer(tab[1]), " prov.)")
  })

  output$cl_mapa <- renderLeaflet({
    d <- datos_cl_mat()
    km <- datos_cl_km()$km
    df <- d$geo %>% mutate(Cluster = factor(paste0("Cluster ", km$cluster)))
    mapa_datos <- mapa_provincias %>% left_join(df, by = c("NAME_2" = "Provincia"))
    pal <- colorFactor(palette = cluster_colores[seq_len(input$cl_k)],
                       domain = levels(df$Cluster), na.color = "#E0E0E0")
    etiquetas <- sprintf("<strong>%s</strong><br/>%s", mapa_datos$NAME_2,
                         as.character(mapa_datos$Cluster)) %>% lapply(htmltools::HTML)
    leaflet(mapa_datos) %>%
      addTiles(urlTemplate = "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",
               attribution = "&copy; OpenStreetMap contributors") %>%
      addPolygons(fillColor = ~pal(Cluster), weight = 1, color = "white",
                  fillOpacity = 0.8, label = etiquetas) %>%
      addLegend(pal = pal, values = levels(df$Cluster), opacity = 0.8,
                title = "Cluster", position = "bottomright")
  })

  output$cl_codo <- renderPlotly({
    codo <- datos_cl_codo()
    req(nrow(codo) > 0)
    mejor <- mejor_k_silueta(codo)
  ...
  output$cl_perfiles <- renderPlotly({
    d <- datos_cl_mat()
    km <- datos_cl_km()$km
    req(nrow(d$mat) > 0)
    medias <- aggregate(d$mat, by = list(Cluster = km$cluster), FUN = mean)
    nac <- colMeans(d$mat)
    logr <- log2(sweep(as.matrix(medias[, -1, drop = FALSE]), 2, nac, `/`))
    logr[!is.finite(logr)] <- 0
    rownames(logr) <- paste0("Cluster ", medias$Cluster)
